...
export default {
  ...
  methods: {
    modifyHead (file) {
      ...
      promise.then((base64) => {
        axios.post('/login/modifyHeadImg', {headImg: base64, userName: vm.userName}).then((response) => {
    ...
    modifyPassword (formName) {
      ...
      vm.$refs[formName].validate((valid) => {
        if (valid) {
          axios.post('/login/modifyPassword', {userName: vm.userName, password: vm.ruleForm.pass}).then((response) => {
            if (response.data.success) {
              vm.$notify({
                title: '操作提示',
                message: `修改密码成功`,
    ...
    loginOut () {
      ...
      vm.$confirm(`是否退出登录？`, '提示', {
        ...
        axios.post('/login/loginOut', {userName: vm.userName}).then((response) => {
          if (response.data.success) {
    ...
    querySession () {
      const vm = this
      axios.post('/login/querySession', {userName: vm.userName}).then((response) => {
        if (!response.data.success) {
