<template ref="login">
  <div class="login">
    <div class="login-main" v-if="isRegister">
      <el-row>
        <el-col :span="24">
          <el-input v-model="userModel.userName" class="trans-input"
            placeholder="请输入账号"
    ...
    <el-dialog class="trans-dialog" title="-" :visible.sync="dialogVisible" :close-on-click-modal="false"
      ...
      <el-form :model="registerModel" :rules="registerRules" ref="registerForm">
        <el-form-item prop="userName">
          <el-col :span="24">
            <el-input v-model="registerModel.userName" class="trans-input"
                      placeholder="请输入账号"
                      prefix-icon="fa fa-fw fa-user-o">
            </el-input>
          </el-col>
...
export default {
  ...
  methods: {
    login () {
      ...
      axios.post('/login/loginValidate', vm.userModel).then((response) => {
        if (response.data.success) {
          ...
          window.sessionStorage.setItem('headImg', response.data.data.headImg)
    ...
    saveUser () {
      ...
      vm.$refs['registerForm'].validate((valid) => {
        ...
        if (vm.registerModel.password === vm.registerModel.password2) {
          axios.post('/login/register', vm.registerModel).then((response) => {
            if (response.data.success) {
              vm.$confirm(`是否直接登录？`, `注册成功`, {
                ...
                vm.$set(vm.userModel, 'userName', vm.registerModel.userName)
